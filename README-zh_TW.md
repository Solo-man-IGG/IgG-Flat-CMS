...
一切起因只是我老婆不會 Markdown + FTP，每次更新網站都要靠我。  
於是我從寫一個簡單的後台開始，邊用邊改。

我花了整整一個星期，越做越完整。  
結果在準備上傳 GitHub 時才發現——**全部加起來竟然只有不到 1MB**，連我自己都嚇到。

最後做出了一套真正輕量、好用、零資料庫的 Flat File CMS。
...
- **靜態頁面**：自定義靜態頁面
- **全文搜尋**：站內搜尋功能，自動建立 JSON 索引，內容異動時自動重建
- **網站標語**：後台可設定全域標語，各文章可自訂副標題覆蓋
...
- **響應式設計**：支援 RWD 行動裝置瀏覽
- **多語系支援**：YAML 語言包架構，一鍵切換語系；可擴充任意語言（正體中文、英文已內建）；支援後台覆蓋特定字串
...
├── cache/                    # 快取目錄
│   ├── pages/                # HTML 頁面快取
│   └── search_index.json     # 搜尋索引（自動產生）
├── data/                     # 語系資料
│   ├── default_lang.yaml     # 預設語系（英文基底）
│   ├── custom_lang.yaml      # 使用者自訂覆蓋
│   ├── active_lang           # 當前語系代碼
│   └── lang/                 # 語言包目錄
│       ├── en.yaml           # 英文語言包
│       └── zh-TW.yaml        # 正體中文語言包
...
| 內部文件 | 公司內部知識庫／文件管理 |
| 使用者管理 | 新增、編輯、刪除管理員帳號 |
| 系統設定 | 配置網站標題、網站標語、選單、郵件設定、首頁 |
| 主題設定 | 自訂主題顏色與導覽樣式 |
| 語系管理 | 一鍵切換語系、新增語言包、手動覆蓋特定字串 |
| 數位簽章 | 設定數位簽章文字 |
...
```
default_lang.yaml  →  data/lang/{語系}.yaml  →  custom_lang.yaml
（英文基底）          （語言包）                    （使用者覆蓋）
```

### 一鍵切換語系

在後台「語系管理」頁面上方的下拉選單選擇語系，點擊「套用」即可立即切換全站（含前台與後台）語言。

### 新增語言

在 `data/lang/` 目錄下新增一個 `.yaml` 檔案，第一行定義語言顯示名稱：
...
系統會自動偵測並出現在後台語系下拉選單中。可參考現有 `zh-TW.yaml` 的格式。

### 自訂覆蓋

在「語系管理」頁面，可針對特定字串手動輸入覆蓋值（顯示為藍色邊框），不會被切換語系影響，適合修改少量用語或修正翻譯。
...
- **搜尋索引**（`cache/search_index.json`）：內容有異動時自動重建，無需手動維護
